...
# Filter significant interactions (e.g., p-value < 0.05)
access_significant_interactions <- subset(access_interaction_results, p_value < 0.05)
significant_interactions

# visualize significant interactions
ggplot(access_data, aes(x = ACS_prop_race_alone_asian, y = CDC_ACCESS2, color = factor(ACS_prop_hispanic_latino > mean(ACS_prop_hispanic_latino)))) +
  geom_point() +
  geom_smooth(method = "lm", formula = y ~ poly(x, 2)) +
  labs(title = "Interaction Between var1 and var2", color = "High/Low var2")
...
# Define bins with specific breaks
breaks <- cut(access_data$ACS_prop_race_alone_asian, breaks = 10)

# View the data
head(data)

data <- access_data %>%
  mutate(bucket = breaks) %>%
  group_by(REGION, bucket) %>%
  summarise(mean_access = mean(CDC_ACCESS2))

ggplot(data, aes(x = bucket, y = mean_access, group = REGION, color = REGION)) +
  geom_line(size = 1) +
  labs(
